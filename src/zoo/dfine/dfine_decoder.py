...
class DFINETransformer(nn.Module):
    ...
    def __init__(
        ...
        self.scale_prior_manager = scale_prior_manager
        self.hidden_dim = hidden_dim
    ...
    def _get_decoder_input(
        ...
        if (
                self.scale_prior_manager is not None
                and self.scale_prior_manager.use_scale_prior
        ):
            # 获取每个 query 的预测类别（argmax）
            # enc_topk_logits: [B, N, num_classes]
            pred_classes = enc_topk_logits.argmax(dim=-1)  # [B, N]

            # 获取图像尺寸（用于归一化）
            if self.eval_spatial_size is not None:
                img_h, img_w = self.eval_spatial_size
            else:
                img_h = img_w = 640.0  # fallback

            # 当前 bbox 尺寸（归一化）
            current_bbox = torch.sigmoid(enc_topk_bbox_unact)  # [B, N, 4]
            current_cxcy = current_bbox[..., :2]
            current_wh = current_bbox[..., 2:]  # [B, N, 2]

            # 构建修正后的 wh
            new_wh = current_wh.clone()

            # 遍历 batch 和 query（可向量化，但为清晰用循环）
            B, N = pred_classes.shape
            for b in range(B):
                for i in range(N):
                    cls_id = int(pred_classes[b, i].item())
                    median_s = self.scale_prior_manager.get_median_scale(cls_id)
                    mean_ar = self.scale_prior_manager.get_mean_ar(cls_id)
                    # 典型宽高（像素）
                    typical_w = math.sqrt(median_s * mean_ar)
                    typical_h = math.sqrt(median_s / mean_ar)

                    # 归一化到 [0,1]
                    norm_w = typical_w / img_w
                    norm_h = typical_h / img_h

                    # 温和混合（alpha 控制强度）
                    alpha = 0.2  # 建议 0.1~0.3
                    blended_w = alpha * norm_w + (1 - alpha) * current_wh[b, i, 0].item()
                    blended_h = alpha * norm_h + (1 - alpha) * current_wh[b, i, 1].item()

                    new_wh[b, i, 0] = blended_w
                    new_wh[b, i, 1] = blended_h

            # 转回 logit space
            new_wh = torch.clamp(new_wh, 1e-6, 1 - 1e-6)
            new_wh_logit = torch.log(new_wh / (1 - new_wh))

            # 替换原 bbox 的 w, h
            enc_topk_bbox_unact = torch.cat([
                enc_topk_bbox_unact[..., :2],
                new_wh_logit
            ], dim=-1)
